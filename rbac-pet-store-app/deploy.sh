...
if [[ $# -ge 1 && $# -le 2 ]]; then
  ...
  echo "=== PetStore App Deployment (RBAC Version) using AWS profile: $AWS_PROFILE ==="
  ...
  echo "=== PetStore App Deployment (RBAC Version) using default AWS profile ==="
  # Get region from default profile
  REGION=$(aws configure get region 2>/dev/null || echo "us-east-1")
  echo "Using AWS Region from default profile: $REGION"
fi

echo "This script will deploy the complete PetStore application stack with RBAC"

# Set variables
STACK_NAME="PetStore-RBAC-App"
...
echo "PetStore application is now available at:"
